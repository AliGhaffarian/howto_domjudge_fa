...
1. docker
2. cgroup(روی لینوکس اراءه میشه)
...
## پنل ادمین
### اضافه کردن مسابقه
لینک پنل: http://domserver/jury/contests  
اینجا لیست کانتست ها رو میتونین ببینین و ادیت کنین، اسکوربورد هارو فریز و آن فریز کنین(برای آن فریز باید داخل قسمت ادیت اون کانتست تایم فریزشو پاک کنین)، و پایین پیج هم گزینه اضافه کردن کانتست وجود داره 
####  تنظیم زمان بندی کانتست
موقع اضافه کردن کانتست یسری فیلد ها مربوط به زمان بندی هستن که بعضی اوقات میتونه گیج کننده بشه.
...
که برای تایم زون ایران میتونین از `Asia/Tehran` استفاده کنین
...
### اضافه کردن سوال
هر سوال شامل یک زیپ فایل میشه که اگه خودمون رو درگیر تنظیمات زیاد دام جاج نکنیم به این ساختار هستش(میتونین دستی هم سوالات رو اضافه کنین داخل رابط وب)  
پیش میاد که هر دو فایل از اپشن خاصی استفاده کنن، اینجا در موردشون صحبت نشده
...
```
#### تست کیس ها
میرن داخل `data/secret`. هر ورودی میره داخل `number.in` و هر خروجی میره داخل `number.ans` و برای هر جفت ورودی و خروجی که به هم مربوط هستن شمارشون یکیه  
#### problem.pdf
...
```
##### محدودیت زمانی
فایل مربوطه:  `domjudge-problem.ini
نمونه:
```ini
timelimit='2'
```
...
برای اینکه تصحیح تست کیس تاثیر پیدا کنه باید ۲ مرحله رو طی کنین
#### تصحیح ورودی و خروجی تست کیس ها
- وارد پنل ادیت سوال مورد نظر بشین
- حالا باید قسمتی به اسم `Testcases` رو ببینین. `edit` رو انتخاب کنین
- میتونین ورودی و خروجی تست کیس هارو عوض کنین، یا حتی تست کیس جدید اضافه کنین
#### جاج کردن دوباره سوالات
لینک پنل: http://domserver/jury/rejudgings
...
### اضافه کردن یوزر
لینک پنل: http://domserver/jury/users
...
#### اضافه کردن تیم
لینک پنل: http://domserver/jury/teams
...
دام سرور بهتون هشدار میده که یسری پارامترای دیتا بیس بد ست شدن(هشدار های مربوط به تنظیمات دام جاج رو از http://domserver/jury/config/check میتونین چک کنین). 
برای رفع این هشدار ها میتونین:  
1. از کانتینر دیتا بیس تون شل بگیرین  
```bash
...
لینک پنل: http://domserver/jury/clarifications/send
